...
describe("M009 safe state and child fencing", () => {
  ...
  it("requires bounded owners and a single final fence containing child resources", async () => {
    const { loadBoundedClientServiceSection } = await import(
      "../../packages/client-services/src/query-service.ts"
    );
    const signalSeen = vi.fn();
    const result = await loadBoundedClientServiceSection(
      {
        load: ({ signal }) =>
          new Promise((resolve) => {
            signalSeen(signal);
            setTimeout(
              () =>
                resolve({
                  section: { state: "empty", generatedAt: "2026-08-21T00:00:00Z" },
                  sourceVersion: "v1",
                  resourceFences: [],
                }),
              50,
  ...
  it.each([
    ...
    [
      "mismatched",
      [{ internalResourceId: "child-1", resourceEpoch: 1, sourceVersion: "other.v1" }],
    ],
    [
      "duplicate",
      [
        { internalResourceId: "child-1", resourceEpoch: 1, sourceVersion: "timeline.v1" },
        { internalResourceId: "child-1", resourceEpoch: 1, sourceVersion: "timeline.v1" },
      ],
    ],
