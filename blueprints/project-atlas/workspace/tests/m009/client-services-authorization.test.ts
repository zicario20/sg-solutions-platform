...
function dependencies(revalidate: boolean, items = [syntheticM009Root()]) {
  return {
    auth: { authorize: vi.fn().mockResolvedValue({ kind: "authorized", snapshot: syntheticM009Snapshot }), revalidate: vi.fn().mockResolvedValue(revalidate) },
    source: { list: vi.fn().mockResolvedValue({ state: "fresh", generatedAt: M009_TEST_NOW, items }), detail: vi.fn(), verifyFinalFence: vi.fn().mockResolvedValue(true) },
    sections: {}, now: () => M009_TEST_NOW
  } as const;
}

describe("M009 resource authorization", () => {
  it("returns only ownership-matched active grants", async () => {
    const service = new ClientServicesQueryService(dependencies(true, [syntheticM009Root(), syntheticM009Root({ ownerAccountId: "other" })]));
