...
const card = {
  ...
  context: { type: "organization", label: "Acme LLC" },
  ...
  currentMilestone: { label: "Documents reviewed", stateLabel: "Completed" },
  milestones: { completed: 1, total: 3 },
  nextStepLabel: "Review the request",
...
describe("M009 public context, milestones and accessibility", () => {
  it("accepts public display content and rejects internal keys", () => {
    ...
    ).toBe("Tax preparation");
    expect(() =>
      parseClientServiceListDto({
        schemaVersion: "m009.list.v2",
        context: card.context,
        items: [{ ...card, definitionKey: "service.tax" }],
      }),
    ).toThrow();
  });
  ...
  it("renders organization context, milestones, canonical routes and unique focusable actions", () => {
    const html = renderToStaticMarkup(
      <ClientServicesDirectory locale="en" state="ready" context={card.context} items={[card]} />,
    );
    ...
    expect(
      readFileSync(
        resolve(
          import.meta.dirname,
          "../../packages/ui/src/client-services/ClientServices.module.css",
        ),
        "utf8",
      ),
  ...
  it.each([
    ["es", "personal", "Personal", "Mis servicios"],
    ["es", "organization", "Empresa", "Mis servicios"],
    ["en", "personal", "Personal", "My services"],
    ["en", "organization", "Company", "My services"],
  ] as const)("renders %s %s context", (locale, type, label, title) => {
  ...
  it.each(["empty", "filter-empty", "partial", "stale", "unavailable"] as const)(
    "renders the %s route state in ES and EN",
    (state) => {
