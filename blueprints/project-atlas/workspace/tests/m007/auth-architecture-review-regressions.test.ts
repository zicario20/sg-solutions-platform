...
describe("M007 architecture-review security regressions", () => {
  it("dispatches a route-specific backend facade with the configured canonical origin", async () => {
    const runtime = createServerAuthRuntime({
      ...
      controlPlane: {
        admit: async () => ({ kind: "accepted" }),
        revokeCurrent: async () => ({ kind: "denied" }),
        revokeOthers: async () => ({ kind: "denied" }),
      },
      emailAuth: {
    ...
    const response = await createAuthRouteHandler(
      runtime,
      "register",
    )(
  ...
  it("rejects unsafe OAuth returns and provider results without verified claims", async () => {
    ...
    await expect(
      oauth.begin({
        ...
        returnIntent: "//attacker.example",
        browserBinding: "browser",
      }),
    ).rejects.toThrow("OAUTH_RETURN_INTENT_DENIED");
    await expect(
      createSupabaseIdentityProvider({ runtimeState: "disabled" }).completeGoogle({}),
  ...
  it("authorizes only a current server-derived grant and rejects supplied claims", async () => {
    const authorization = new AuthoritativeAuthorizationService({
      load: async () => ({
        activeSession: true,
        accountId: "a",
        accessEpoch: 2,
        ...
        permissions: ["client.case.read"],
        resource: { accountId: "a", organizationId: "o", accessEpoch: 2, policyEpoch: 2 },
      }),
    });
    await expect(
      ...
      new AuthorizationService().authorize(
        {
          ...
          assurance: "aal2",
          resourceReceipt: { accountId: "a", organizationId: "o", accessVersion: 1 },
        },
        "admin.user.manage",
      ),
  ...
  it("requires a provider-verified subject receipt before creating a local account mirror", async () => {
    await expect(
      ...
      } as never).registerProspect({ subject: "untrusted" }),
    ).resolves.toEqual({ kind: "denied" });
  });
  ...
  it("uses owner evidence for CRM links and retains conflicts for manual review", async () => {
    const linking = new PartyLinkingService(
      { resolve: async () => ({ kind: "conflict" }) },
      { loadCrmReceipt: async () => undefined },
    );
    await expect(linking.link({ accountId: "account-1", evidenceId: "unknown" })).resolves.toEqual({
      kind: "manual_review",
    });
  });
  ...
  it("fails closed when durable rate, audit, or outbox ports are unavailable", async () => {
    const controls = createTransactionalAuthControls();
    await expect(controls.admit({ purpose: "login", identifierDigest: "digest" })).resolves.toEqual(
      { kind: "unavailable" },
    );
  });
