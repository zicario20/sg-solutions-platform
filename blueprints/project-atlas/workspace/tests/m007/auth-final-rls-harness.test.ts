...
import { M007_MIGRATION_FILES, runM007RlsHarness } from "../support/m007-auth-rls-harness.mjs";
  ...
  it("applies the final forward-only migration and exercises final repositories without direct DML", async () => {
    ...
    const executor = {
      ...
      execute: async (operation: string) => {
        ...
        if (operation === "global_table_denied" || operation === "global_table_access")
          return "denied";
    ...
    await runM007RlsHarness({
      executor,
      migrationSources: M007_MIGRATION_FILES.map((file) => ({ file, sql: "select 1" })),
    });

    expect(M007_MIGRATION_FILES.at(-1)).toBe("0035_m007_rate_audit_outcomes.sql");
