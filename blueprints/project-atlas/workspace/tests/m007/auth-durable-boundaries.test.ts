...
describe("M007 durable server boundaries", () => {
    ...
    const oauth = createDurableOAuthTransactionService({
      ...
      consume: async (input) => {
        if (
          !issued ||
          consumed ||
          input.stateDigest !== issued.stateDigest ||
          input.nonceDigest !== issued.nonceDigest ||
          input.pkceVerifierDigest !== issued.pkceVerifierDigest
        )
  ...
  it("loads immutable server evidence by ID and never accepts a request-shaped receipt", async () => {
    ...
    const accounts = new AccountService(
      {
        createProspect: async (subject: string) => {
          created += 1;
          return { id: "a", subject, status: "pending_verification" as const };
        },
      } as never,
      {
        loadSupabaseReceipt: async (id) =>
          id === "supabase-1"
            ? { subject: "subject-1", verifiedAt: Date.now(), issuer: "supabase" }
            : undefined,
      },
    );
    await expect(
      accounts.registerProspect({
        subject: "subject-1",
        evidenceId: "fabricated",
        verifiedSubjectReceipt: { subject: "subject-1", verifiedAt: Date.now() },
      }),
    ).resolves.toEqual({ kind: "denied" });
    await expect(
      accounts.registerProspect({ subject: "subject-1", evidenceId: "supabase-1" }),
    ).resolves.toMatchObject({ status: "pending_verification" });
    expect(created).toBe(1);
    const linking = new PartyLinkingService(
      { resolve: async () => ({ kind: "linked" as const, relationshipReceipt: "crm-link" }) },
      {
        loadCrmReceipt: async (id) =>
          id === "crm-1" ? { evidenceId: id, verifiedAt: Date.now() } : undefined,
      },
    );
    await expect(linking.link({ accountId: "a", evidenceId: "fabricated" })).resolves.toEqual({
      kind: "manual_review",
    });
    await expect(linking.link({ accountId: "a", evidenceId: "crm-1" })).resolves.toEqual({
      kind: "linked",
      relationshipReceipt: "crm-link",
    });
  });
