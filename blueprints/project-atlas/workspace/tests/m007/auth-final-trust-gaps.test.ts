...
class FakeSql implements AuthSql {
  ...
  async begin<T>(
    callback: (transaction: {
      unsafe<R>(query: string, parameters?: readonly unknown[]): Promise<R>;
    }) => Promise<T>,
  ): Promise<T> {
    return callback({
      unsafe: async <R>(query: string, parameters: readonly unknown[] = []): Promise<R> => {
        ...
        if (
          query.includes("atlas_auth_revoke_current") ||
          query.includes("atlas_auth_revoke_others")
        )
          return [{ revoked: true }] as R;
        if (query.includes("atlas_auth_consume_oauth_transaction"))
          return [{ outcome: "consumed" }] as R;
        if (query.includes("atlas_auth_authenticate_identity"))
        ...
        if (query.includes("select id, provider_subject"))
          return [{ id: "evidence-1", provider_subject: "subject-1" }] as R;
        if (query.includes("select id, resolution"))
          return [{ id: "crm-1", resolution: "linked", relationship_receipt: "receipt-1" }] as R;
        if (query.includes("insert into auth_accounts"))
          return [{ id: "account-1", status: "active" }] as R;
        if (query.includes("insert into auth_external_identities"))
          return [{ id: "external-1", account_id: "account-1" }] as R;
...
describe("AR-001 canonical session and CSRF lifecycle", () => {
  ...
  it("emits a session-bound readable CSRF cookie beside the HttpOnly auth cookie", () => {
    ...
    expect(headers[1]).toContain(
      `__Host-atlas_csrf=${deriveSessionCsrfToken("csrf-secret-at-least-32-bytes-long", "raw-session-handle")}`,
    );
...
describe("AR-002 and AR-005 restricted PostgreSQL repositories", () => {
  it("uses narrow OAuth and identity functions instead of direct table DML", async () => {
    ...
    await oauth.issue({
      ...
      purpose: "sign_in",
      provider: "google",
      stateDigest: "s".repeat(32),
      nonceDigest: "n".repeat(32),
      pkceVerifierDigest: "p".repeat(32),
      browserBindingDigest: "b".repeat(32),
      redirectHash: "r".repeat(32),
      returnIntent: "/client",
      callbackUrl: "https://app.example/api/auth/oauth/google/callback",
    ...
    await identities.storeSupabaseEvidence({
      id: "evidence-1",
      identity: {
        provider: "google",
        issuer: "https://issuer.example",
    ...
    await identities.storeCrmEvidence({
      id: "crm-1",
      supabaseEvidenceId: "evidence-1",
      resolution: { kind: "linked", partyId: "party-1", relationshipReceipt: "receipt-1" },
      verifiedAt: now,
      expiresAt: new Date(now.getTime() + 60_000),
    });
    await identities.authenticate({
      supabaseEvidenceId: "evidence-1",
      crmEvidenceId: "crm-1",
      expectedIssuer: "https://issuer.example",
      expectedAudience: "atlas",
      accountId: "account-1",
      externalIdentityId: "external-1",
      partyLinkId: "party-link-1",
      conflictId: "conflict-1",
      session: {
        id: "session-1",
        handleDigest: "h".repeat(32),
        familyId: "family-1",
        assurance: "aal1",
    ...
    expect(
      sql.statements.every((statement) => /^select (?:\* from )?atlas_auth_/u.test(statement)),
    ).toBe(true);
  ...
  it("uses narrow invitation, audit and outbox functions instead of direct table DML", async () => {
    ...
    await invitations.consume({
      id: "invitation-1",
      proofDigest: "p".repeat(32),
      sessionHandleDigest: "s".repeat(32),
      now,
    });

    const controls = new PostgresDurableAuthControlsRepository(sql);
    await controls.appendAudit({
      eventKey: "event-key-0000001",
      eventName: "sign_in",
      outcome: "accepted",
      correlationId: "correlation-00001",
      metadata: { outcome: "accepted" },
      now,
    });
    await controls.admitAndEnqueue({
      action: "sign_in",
      riskKeyDigests: ["a".repeat(32), "b".repeat(32)],
      threshold: 5,
      windowSeconds: 60,
      eventKey: "event-key-0000002",
      correlationId: "correlation-00002",
      metadata: { outcome: "attempted" },
      now,
    });
    await controls.lease({
      owner: "worker-00000001",
  ...
  it("constructs concrete SQL, provider and CRM ports only from complete server configuration", () => {
    ...
    const factories = {
      createSql: (databaseUrl: string) => {
        calls.push(`sql:${databaseUrl}`);
        return new FakeSql();
      },
      createProvider: () => {
        calls.push("provider");
        return { verifyGoogle: async () => undefined };
      },
      createCrm: () => {
        calls.push("crm");
        return { resolve: async () => ({ kind: "unavailable" as const }) };
      },
    };
...
describe("AR-006 invitation subject binding", () => {
  it("fixes the expected provider subject at issue and consumes using only server session evidence", async () => {
    ...
    const service = createDurableInvitationService(
      {
        issue: async (input) => {
          issuedSubject = input.expectedProviderSubject;
        },
        consume: async (input) => {
          consumed = input;
    ...
    const invitation = await service.issue({
      contactId: "contact-1",
      scope: "client",
      inviterAccountId: "account-1",
      expectedProviderSubject: "subject-1",
    ...
    await service.consume({
      ...
      sessionHandle: "raw-session-handle",
    });

    expect(issuedSubject).toBe("subject-1");
    expect(consumed).toMatchObject({
      id: invitation.id,
      sessionHandleDigest: digestOpaqueProof("raw-session-handle"),
    });
