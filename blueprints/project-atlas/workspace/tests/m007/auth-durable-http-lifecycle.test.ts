...
describe("M007 durable HTTP lifecycle wiring", () => {
  it("routes session listing, rotation, and invitation acceptance through the injected durable adapter", async () => {
    ...
    await expect(handler.acceptInvitation(new Request("https://portal.example/api/auth/invitations/accept", { method: "POST", headers: { origin: "https://portal.example", cookie: "__Host-atlas_auth=handle", "content-type": "application/x-www-form-urlencoded" }, body: "id=i&proof=p" }))).resolves.toMatchObject({ status: 202 });
