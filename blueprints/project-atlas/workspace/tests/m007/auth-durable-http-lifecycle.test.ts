...
describe("M007 durable HTTP lifecycle wiring", () => {
  it("routes session listing, rotation, and invitation acceptance through the injected durable adapter", async () => {
    ...
    const handler = createDurableAuthHttpFactory({
      ...
      invitations: { accept: async (input) => { calls.push(input.sessionHandle); return { kind: "consumed" as const }; } },
    }, "csrf-secret-at-least-32-bytes-long", { canonicalOrigin: "https://portal.example" });
    ...
    await expect(handler.acceptInvitation(new Request("https://portal.example/api/auth/invitations/accept", { method: "POST", headers: { origin: "https://portal.example", cookie: `__Host-atlas_auth=handle; __Host-atlas_csrf=${csrf}`, "content-type": "application/x-www-form-urlencoded" }, body: `id=i&code=p&csrf=${csrf}` }))).resolves.toMatchObject({ status: 202 });
