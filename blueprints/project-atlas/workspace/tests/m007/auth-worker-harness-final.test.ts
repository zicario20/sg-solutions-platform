...
describe("AR-009 worker and live harness assertions", () => {
  ...
  it("does not dispatch a command whose lease owner, purpose, or token was not claimed", async () => {
    ...
    const worker = createDurableAuthOutboxWorker({
      ...
      timeoutMs: 100,
      repository: {
        recoverExpiredLeases: async () => ({ dispatchToReconcile: 0, reconcileToManualReview: 0 }),
        lease: async () => [
          {
            commandId: "command-1",
            purpose: "email",
        ...
        recordDispatchOutcome: async () => undefined,
      ...
      provider: {
        ...
        queryByOwner: async () => ({ outcome: "unknown" }),
      },
    });
  ...
  it("executes database assertions for audit/outbox policies and fails an empty worker lease", async () => {
    ...
    expect(
      statements.some(
        (statement) =>
          statement.includes("to_regprocedure") &&
          statement.includes("atlas_auth_issue_oauth_transaction"),
      ),
    ).toBe(true);
