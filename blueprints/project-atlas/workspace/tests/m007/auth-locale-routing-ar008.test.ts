...
describe("AR-008 auth locale routing", () => {
  ...
  it("sets a validated locale cookie and redirects once only to a safe portal route", async () => {
    ...
    const response = await handler(new Request("https://portal.example/api/auth/locale", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: "locale=en&return_to=%2Fclient%2Fsecurity" }));
    ...
    const loop = await handler(new Request("https://portal.example/api/auth/locale", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: "locale=xx&return_to=%2Fapi%2Fauth%2Flocale" }));
