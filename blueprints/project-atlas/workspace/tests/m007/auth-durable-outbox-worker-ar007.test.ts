import { createDurableAuthOutboxWorker, type DurableAuthOutboxCommand, type DurableAuthOutboxRepository } from "@atlas/auth";
...
class FakeOutboxRepository implements DurableAuthOutboxRepository {
  ...
  async recoverExpiredLeases() {
    ...
    return { dispatchToReconcile: 0, reconcileToManualReview: this.recoverMode === "expired_reconcile" ? 1 : 0 };
  }
  ...
  async recordDispatchOutcome(input: { outcome: "sent" | "failed" | "unknown" }) {
    this.state = input.outcome === "sent" ? "completed" : input.outcome === "failed" ? "pending" : "reconciling";
  }

  async recordReconciliation(input: { outcome: "sent" | "failed" | "unknown" }) {
    this.state = input.outcome === "sent" ? "completed" : input.outcome === "failed" ? "pending" : "manual_review";
  }
}
...
describe("AR-007 durable outbox worker", () => {
  it("reconciles an unknown owner outcome and never blindly dispatches it again", async () => {
    ...
    let ownerQueries = 0;
    const provider = {
      send: async () => { sends += 1; return { outcome: "unknown" as const }; },
      queryByOwner: async () => { ownerQueries += 1; return { outcome: "unknown" as const }; },
    };

    await createDurableAuthOutboxWorker({ repository, provider, owner: "worker-1", leasePurpose: "dispatch", maxJobs: 2, timeoutMs: 1_000 }).run();
    await createDurableAuthOutboxWorker({ repository, provider, owner: "worker-1", leasePurpose: "dispatch", maxJobs: 2, timeoutMs: 1_000 }).run();
    await createDurableAuthOutboxWorker({ repository, provider, owner: "worker-2", leasePurpose: "reconcile", maxJobs: 2, timeoutMs: 1_000 }).run();
  ...
  it("moves an expired reconciliation lease to manual review without calling the provider", async () => {
    ...
    let providerCalls = 0;
    const provider = { send: async () => { providerCalls += 1; return { outcome: "sent" as const }; }, queryByOwner: async () => { providerCalls += 1; return { outcome: "sent" as const }; } };

    await createDurableAuthOutboxWorker({ repository, provider, owner: "worker-2", leasePurpose: "reconcile", maxJobs: 1, timeoutMs: 1_000 }).run();
