...
describe("M007 PostgreSQL auth entrypoint wiring", () => {
  it("uses the injected durable adapter for real session and invitation routes", async () => {
    ...
    const csrf = deriveSessionCsrfToken(secret, "h");
    const entrypoints = createAuthEntryPoints(
      async () => ({
        sessions: {
          list: async () => {
            calls.push("list");
          ...
          rotate: async () => {
            calls.push("rotate");
            return { kind: "family_revoked" as const };
          },
    ...
    const session = await entrypoints.get(
      new Request("https://portal.example/api/auth/sessions", {
        headers: {
          origin: "https://portal.example",
          cookie: `__Host-atlas_auth=h; __Host-atlas_csrf=${csrf}`,
          "x-atlas-csrf": csrf,
        },
      }),
    );
    const rotate = await entrypoints.post(
      new Request("https://portal.example/api/auth/sessions", {
        method: "POST",
        headers: {
          origin: "https://portal.example",
          cookie: `__Host-atlas_auth=h; __Host-atlas_csrf=${csrf}`,
          "x-atlas-csrf": csrf,
          "x-atlas-session-action": "rotate",
        },
      }),
    );
    const invite = await entrypoints.post(
    ...
    expect(calls).toEqual(["list", "rotate", "accept"]);
  });
});
