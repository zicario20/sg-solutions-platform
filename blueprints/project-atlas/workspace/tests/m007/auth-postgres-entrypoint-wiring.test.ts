...
describe("M007 PostgreSQL auth entrypoint wiring", () => {
  it("uses the injected durable adapter for real session and invitation routes", async () => {
    ...
    const entrypoints = createAuthEntryPoints(async () => ({ sessions: { list: async () => { calls.push("list"); return []; }, rotate: async () => { calls.push("rotate"); return { kind: "family_revoked" as const }; }, revokeCurrent: async () => ({ kind: "revoked" as const }), revokeOthers: async () => ({ kind: "revoked" as const }) }, invitations: { accept: async () => { calls.push("accept"); return { kind: "consumed" as const }; } } }), secret);
    ...
    const invite = await entrypoints.post(new Request("https://portal.example/api/auth/invitations/accept", { method: "POST", headers: { origin: "https://portal.example", cookie: "__Host-atlas_auth=h", "content-type": "application/x-www-form-urlencoded" }, body: "id=i&proof=p" }));
