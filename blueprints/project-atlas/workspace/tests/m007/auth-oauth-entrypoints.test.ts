...
describe("M007 OAuth and invitation entrypoints", () => {
  it("handles disabled, replay, conflict, and verified callback through server adapters", async () => {
    ...
    const routes = createOAuthInvitationEntryPoints(
      {
        start: async () => ({
          ...
          location: "https://supabase.example/auth/v1/authorize?state=s",
        }),
        callback: async () => {
          if (used) return { kind: "denied" as const };
          used = true;
          return { kind: "authenticated" as const, handle: "h" };
        },
    ...
    await expect(
      routes.callback(
        new Request(
          "https://portal.example/api/auth/oauth/google/callback?state=s&code=provider-code",
          { headers: { origin: "https://portal.example" } },
        ),
      ),
