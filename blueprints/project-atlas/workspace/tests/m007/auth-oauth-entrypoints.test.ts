...
describe("M007 OAuth and invitation entrypoints", () => {
  it("handles disabled, replay, conflict, and verified callback through server adapters", async () => {
    let used = false;
    const routes = createOAuthInvitationEntryPoints({ start: async () => ({ kind: "started" as const, state: "s", nonce: "n", pkceVerifier: "p" }), callback: async () => used ? { kind: "denied" as const } : (used = true, { kind: "authenticated" as const, handle: "h" }), issueInvitation: async () => ({ kind: "issued" as const, id: "i", proof: "p" }) });
