...
describe("M007 MFA and service identities", () => {
  it("fails closed for disabled MFA and over-scoped service credentials", async () => {
    ...
    await expect(new ServiceIdentityVerifier().verify({ audience: "voice", scopes: ["voice.read", "admin.user.manage"] }, { audience: "voice", scopes: ["voice.read"] })).resolves.toEqual({ kind: "denied" });
  });
});
