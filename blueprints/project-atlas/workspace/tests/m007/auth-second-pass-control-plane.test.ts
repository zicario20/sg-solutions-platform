...
describe("M007 durable auth control plane", () => {
  it("binds CSRF to a server session and revokes replayed rotated handles", async () => {
    ...
    await expect(auth.refresh({ handle: session.handle, csrf: session.csrf })).resolves.toEqual({
      kind: "family_revoked",
    });
    await expect(auth.authorize({ handle: rotated.handle, csrf: "wrong" })).resolves.toEqual({
      kind: "denied",
    });
  });
  ...
  it("uses one durable unit for rate admission, audit and outbox", async () => {
    ...
    const auth = new AuthControlPlane(repository);
    await expect(
      auth.admit({ purpose: "recovery", identifier: "user@example.com" }),
  ...
  it("requires a server-verified Supabase callback bound to its transaction", async () => {
    const provider = createOfficialSupabaseIdentityProvider({
      runtimeState: "enabled",
      issuer: "https://issuer.example",
      audience: "atlas-app",
      verifyWithJwks: async () => undefined,
    });
    await expect(
      provider.completeGoogle({ state: "state", nonce: "nonce", pkceVerifier: "pkce" }),
