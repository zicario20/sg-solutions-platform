...
describe("AR-009 live PostgreSQL RLS harness contract", () => {
  it("stays inert without both explicit authorization gates", async () => {
    ...
    expect(isM007HarnessAuthorized({ DATABASE_URL: "postgres://db", M007_RLS_HARNESS: "authorized" })).toBe(true);
  });

  it("applies every M007 migration and executes least-privilege behavioral checks", async () => {
    const { M007_MIGRATION_FILES, runM007RlsHarness } = await import("../support/m007-auth-rls-harness.mjs");
    const events: string[] = [];
    ...
    await expect(runM007RlsHarness({ executor, migrationSources: M007_MIGRATION_FILES.map((file) => ({ file, sql: "select 1" })) })).resolves.toEqual({ kind: "passed", migrationsApplied: M007_MIGRATION_FILES.length });
  ...
  it("fails when cross-account data becomes visible", async () => {
    ...
    await expect(runM007RlsHarness({ executor, migrationSources: M007_MIGRATION_FILES.map((file) => ({ file, sql: "select 1" })) })).rejects.toThrow("M007_CROSS_ACCOUNT_READ_VISIBLE");
  });
});
