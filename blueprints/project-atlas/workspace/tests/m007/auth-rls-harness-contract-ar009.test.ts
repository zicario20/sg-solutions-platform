...
describe("AR-009 live PostgreSQL RLS harness contract", () => {
  ...
  it("applies every M007 migration and executes least-privilege behavioral checks", async () => {
    ...
    await expect(runM007RlsHarness({ executor, migrationSources: Array.from({ length: 11 }, (_, index) => ({ name: `00${23 + index}`, sql: `select ${index + 1}` })) })).resolves.toEqual({ kind: "passed", migrationsApplied: 11 });
  ...
  it("fails when cross-account data becomes visible", async () => {
    ...
    await expect(runM007RlsHarness({ executor, migrationSources: Array.from({ length: 11 }, (_, index) => ({ name: `00${23 + index}`, sql: `select ${index + 1}` })) })).rejects.toThrow("M007_CROSS_ACCOUNT_READ_VISIBLE");
  });
});
