...
const migrationUrl = new URL(
  "../../drizzle/0034_m007_cyber_neo_auth_remediation.sql",
  import.meta.url,
);
const migration = existsSync(migrationUrl) ? readFileSync(migrationUrl, "utf8") : "";
const auditMigrationUrl = new URL(
  "../../drizzle/0035_m007_rate_audit_outcomes.sql",
  import.meta.url,
);
...
const elements = (node: unknown, result: ElementNode[] = []): ElementNode[] => {
  if (Array.isArray(node)) {
    for (const child of node) elements(child, result);
    return result;
  }
...
describe("M007 Cyber Neo remediation", () => {
  it("CN-001 denies a verified authority when the durable account cannot establish a session", async () => {
    ...
    const service = createServerEmailAuthService({
      ...
      repository: {
        consumeProviderToken: async () => true,
        establishSession: async () => ({ kind: "denied" as const }),
    ...
    await expect(
      service.signIn({ email: "person@example.com", password: "long-password" }),
    ).resolves.toEqual({ kind: "accepted", internalOutcome: "denied" });
  ...
  it("CN-002 accepts invitations only by same-origin POST with session-bound CSRF", async () => {
    ...
    const headers = {
      ...
      cookie: `__Host-atlas_auth=session-handle; __Host-atlas_csrf=${csrf}`,
    ...
    await expect(
      handler.acceptInvitation(
        new Request("https://portal.example/api/auth/invitations/accept", {
          ...
          body: `id=invite-1&code=invite-code&csrf=${csrf}`,
        }),
      ),
    ).resolves.toMatchObject({ status: 403 });
    await expect(
  ...
  it("CN-003 fails closed with a neutral response when no trusted risk dimension exists", async () => {
    ...
    const sql = {
      begin: async (
        callback: (transaction: {
          unsafe: <T>(statement: string, parameters?: readonly unknown[]) => Promise<T>;
        }) => Promise<unknown>,
      ) =>
        callback({
          unsafe: async <T>(statement: string, parameters: readonly unknown[] = []) => {
            queries.push({ statement, parameters });
            return statement.includes("append_audit")
              ? ([{ appended: true }] as T)
              : ([{ allowed: true }] as T);
          },
        }),
    ...
    const controlPlane = createConfiguredAuthControlPlane(
      {
        ...
        AUTH_CONTROL_HMAC_KEY: "risk-secret-at-least-32-bytes-long",
    ...
    const routes = createOAuthInvitationEntryPoints(
      {
        start: async () => {
          starts += 1;
          return { kind: "redirect" as const, location: "https://provider.example/authorize" };
        },
    ...
    const response = await routes.start(
      new Request("https://portal.example/api/auth/oauth/google/start", {
        method: "POST",
        headers: { "x-forwarded-for": "203.0.113.8", "x-request-id": "zero-risk-request" },
      }),
    );
    ...
    expect(
      queries.some(
        (query) =>
          query.statement.includes("append_audit") && query.parameters.includes("rate_limited"),
      ),
    ).toBe(true);
  });
  ...
  it("CN-003 derives a flow-specific rate key from verification and reset codes", async () => {
    ...
    const sql = {
      begin: async (
        ...
        callback({
          unsafe: async <T>(statement: string, parameters: readonly unknown[] = []) => {
            if (statement.includes("admit_and_enqueue"))
              (admissions as unknown[][]).push(parameters);
    ...
    const request = (path: string, code: string) =>
    ...
    await expect(runtime.admit("verify", request("verify", "verify-code"))).resolves.toEqual({
      kind: "accepted",
    });
    await expect(runtime.admit("reset", request("reset", "reset-code"))).resolves.toEqual({
      kind: "accepted",
    });
    expect(admissions).toHaveLength(2);
    expect(
      admissions.every((parameters) => Array.isArray(parameters[1]) && parameters[1].length === 1),
    ).toBe(true);
  ...
  it("CN-005 rotates with five SQL arguments and derives the original absolute expiry", async () => {
    ...
    const repository = new PostgresAuthControlPlaneRepository({
      begin: async (callback) =>
        callback({
          unsafe: async <T>(query: string, values: readonly unknown[] = []) => {
            statement = query;
            parameters = values;
            return [{ outcome: "rotated" }] as T;
          },
        }),
    });
    await repository.rotateSession({
      ...
      next: {
        id: "next-session",
        handleDigest: "next-digest",
        idleExpiresAt: new Date("2026-08-21T12:30:00Z"),
      },
  ...
  it("CN-006 renders user-entered POST codes and never reflects URL proofs", () => {
    const invitation = InvitationAcceptView({
      ...uiProps,
      invitation: {
        id: "raw-id",
        proof: "raw-proof",
        contactId: "raw-contact",
        scope: "raw-scope",
        identityEvidenceId: "raw-evidence",
      },
    } as never);
    const names = [
      ResetPasswordView(uiProps as never),
      VerifyEmailView(uiProps as never),
      invitation,
    ].flatMap((view) =>
      elements(view)
    ...
    for (const page of ["reset-password", "verify-email", "invitations/accept"]) {
      const source = readFileSync(
        new URL(`../../apps/app/src/app/client/${page}/page.tsx`, import.meta.url),
        "utf8",
      );
    ...
    const runtime = createServerAuthRuntime({
      ...
      controlPlane: {
        ...
        auditOutcome: async (input) => {
          audited.push(`${input.purpose}:${input.outcome}`);
        },
        revokeCurrent: async () => ({ kind: "revoked" as const }),
        revokeOthers: async () => ({ kind: "revoked" as const }),
      },
      emailAuth: {
        signUp: async () => ({ kind: "accepted", internalOutcome: "accepted" }),
        signIn: async () => ({ kind: "accepted", internalOutcome: "accepted" }),
        sendVerification: async () => ({ kind: "accepted", internalOutcome: "accepted" }),
        consumeVerification: async () => ({ kind: "accepted", internalOutcome: "accepted" }),
        requestRecovery: async () => ({ kind: "accepted", internalOutcome: "accepted" }),
        consumeReset: async () => ({ kind: "accepted", internalOutcome: "accepted" }),
        logout: async () => undefined,
      },
    });
    await runtime.handle(
      ...
      new Request("https://portal.example/api/auth/login", {
        ...
        headers: {
          origin: "https://portal.example",
          "content-type": "application/x-www-form-urlencoded",
          "x-request-id": "request-login-0001",
        },
        body: "email=person%40example.com&password=password",
      }),
    );
    await runtime.handle(
      "logout",
      new Request("https://portal.example/api/auth/logout", {
        ...
        headers: {
          ...
          cookie: `__Host-atlas_auth=session-handle; __Host-atlas_csrf=${csrf}`,
          "content-type": "application/x-www-form-urlencoded",
          "x-request-id": "request-logout-0001",
        },
        body: `csrf=${csrf}`,
      }),
    );
  ...
  it("CN-007 keeps public responses neutral while auditing every internal provider outcome", async () => {
    ...
    const repository = {
      consumeProviderToken: async () => true,
      establishSession: async () => ({ kind: "established" as const, accountId: "account-1" }),
      loadProviderToken: async () => undefined,
      clearProviderToken: async () => undefined,
    };
    const providerBase = {
      signIn: async () => ({ kind: "denied" as const }),
      sendVerification: async () => ({ kind: "accepted" as const }),
      requestRecovery: async () => ({ kind: "accepted" as const }),
      consumeVerification: async () => ({ kind: "denied" as const }),
      consumeRecovery: async () => ({ kind: "denied" as const }),
      updatePassword: async () => ({ kind: "denied" as const }),
      logout: async () => undefined,
    };
    const cases = [
    ...
    for (const [expectedOutcome, signUp] of cases) {
      const service = createServerEmailAuthService({
        provider: { ...providerBase, signUp: signUp as never },
        repository,
        sealProviderToken: (value) => value,
      });
      const runtime = createServerAuthRuntime({
      ...
      const response = await runtime.handle(
        "register",
        new Request("https://portal.example/api/auth/register", {
          method: "POST",
          headers: {
            origin: "https://portal.example",
            "content-type": "application/x-www-form-urlencoded",
          },
          body: "email=person%40example.com&password=long-password",
        }),
      );
      expect(response.status).toBe(202);
      await expect(response.json()).resolves.toEqual({ kind: "accepted" });
    ...
    const authenticated = createServerEmailAuthService({
      provider: {
        ...providerBase,
    ...
    const success = await runtime.handle(
      "login",
      new Request("https://portal.example/api/auth/login", {
        method: "POST",
        headers: {
  ...
  it("CN-008 turns every CRM receipt ownership collision into manual review before session insertion", () => {
    const ownershipCheck = migration.indexOf(
      "v_party_account<>v_account OR v_party_state<>'active'",
    );
