...
const forbidden = /ssn|itin|password|credential|account_number|card_number|tax_document|credit_report|upload/iu;
  ...
  it("contains no restricted, upload or direct payment authority", () => {
    for (const formCode of publicFormRegistry.codes) {
      for (const locale of ["es", "en"] as const) {
        ...
        expect(projection.fields.every((field) => ["public", "basic_personal", "financial"].includes(field.sensitivity))).toBe(true);
  ...
  it("keeps consent purposes separated and supports governed conditional rules", () => {
    const consentTypes = new Set(
      publicFormRegistry.codes.flatMap((code) =>
        getPublishedProjection(code, "en")!.consentRequirements.map((consent) => consent.consentType),
      ),
    );
    expect([...consentTypes]).toEqual(expect.arrayContaining([
      "service_contact",
      "sms_contact",
      "whatsapp_contact",
      "email_marketing",
