...
class FakeTransaction implements PublicFormsTransaction {
  ...
  async unsafe<Row extends Record<string, unknown>[]>(statement: string, parameters: unknown[] = []): Promise<Row> {
...
describe("M006 PostgreSQL public forms repository", () => {
  it("commits a protected submission and replays the durable receipt", async () => {
    ...
    await expect(repository.commitAcceptedSubmission({
      scope: "e".repeat(64),
      reservationId: "form_reservation_0000000001",
      submission,
    })).resolves.toEqual(receipt);

    await expect(repository.reserveOrReplay({
      scope: "e".repeat(64),
      commandDigest: submission.commandDigest,
    ...
    expect(tx.statements.some((statement) => statement.startsWith("insert into form_responses"))).toBe(true);
