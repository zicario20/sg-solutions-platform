...
const action = (type: ClientActionDto["type"], opaqueRef: string, overrides: Partial<ClientActionDto> = {}): ClientActionDto => ({
  type,
  opaqueRef,
  title: "Safe action",
  blocking: false,
  ...
  routeKey: type === "security_identity" ? "security" : type === "blocking_payment" ? "payments" : "services",
  ...overrides,
});

function input(changes: Partial<DashboardPriorityInput> = {}): DashboardPriorityInput {
  return Object.fromEntries(sourceCodes.map((code) => [code, { state: "empty", candidates: [] }]).concat(Object.entries(changes))) as DashboardPriorityInput;
}

describe("M008 deterministic priority", () => {
  it("selects security before payment and task actions", () => {
    const result = selectDashboardPriority(input({
      security: { state: "fresh", candidates: [action("security_identity", "security-1")] },
      payments: { state: "fresh", candidates: [action("blocking_payment", "payment-1", { blocking: true })] },
  ...
  it("does not claim a payment action while security is unavailable", () => {
    expect(selectDashboardPriority(input({
  ...
  it("rejects an action with a route outside the registry", () => {
    expect(() => selectDashboardPriority(input({
      tasks: { state: "fresh", candidates: [action("due_task", "task-1", { routeKey: "admin" as never })] },
  ...
  it("uses stable opaque ID as the final tie-break", () => {
    ...
    const result = selectDashboardPriority(input({ notifications: { state: "fresh", candidates: laterFirst } }));
