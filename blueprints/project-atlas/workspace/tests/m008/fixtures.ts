...
export const syntheticEvidence: DashboardAuthorizationEvidence = Object.freeze({
  ...
  contextOptions: Object.freeze([
    { type: "personal" as const, opaqueRef: "synthetic-context-a", label: "Personal" },
  ]),
...
export function dto(): DashboardDto {
  ...
  return {
    ...
    context: {
      type: "personal",
      selectedOpaqueRef: "synthetic-context-a",
      options: syntheticEvidence.contextOptions,
    },
...
const emptyFragment = (owner: DashboardOwnerCode, snapshotId: string): DashboardOwnerFragment =>
  Object.freeze({
    owner,
    snapshotId,
    ...
    DASHBOARD_OWNER_CODES.map((owner) => [
      ...
      Object.freeze({
        ...
        query: async ({ snapshotId }: { readonly snapshotId: string }) =>
          emptyFragment(owner, snapshotId),
      }),
    ]),
...
export function syntheticAuthPort(mode: "allowed" | "revoked" = "allowed"): DashboardAuthPort {
  return Object.freeze({
    authorize: async ({ sessionHandle, requestedContext }) =>
      ...
      (!requestedContext || requestedContext === "synthetic-context-a")
        ? { kind: "authorized" as const, evidence: syntheticEvidence }
        : { kind: "denied" as const },
    revalidate: async (_snapshot: DashboardAuthorizationSnapshot) =>
      mode === "allowed"
        ? { kind: "authorized" as const, evidence: syntheticEvidence }
        : { kind: "denied" as const },
