...
const unavailable = (): DashboardSection<never> => ({
  state: "unavailable",
  safeReason: "provider_disabled",
});
...
describe("M008 dashboard widgets", () => {
  it("orders priority before services and never invents payment state", () => {
    ...
    expect(html.indexOf('data-widget="priority"')).toBeLessThan(
      html.indexOf('data-widget="services"'),
    );
    for (const widget of [
      "services",
      "tasks",
  ...
  it("uses non-realistic skeletons and an opaque POST context control", () => {
    const skeleton = renderToStaticMarkup(
      createElement(DashboardSkeleton, { locale: "en", rows: 2 }),
    );
    expect(skeleton).not.toMatch(/@|\$|\d{3}[- ]\d{2}[- ]\d{4}/);
    const control = renderToStaticMarkup(
      createElement(ContextSwitchControl, {
        locale: "en",
