...
describe("M008 fail-closed HTTP", () => {
  it("returns 401 without an application session", async () => {
    const response = await dashboardGet(new Request(`${origin}/api/client/dashboard`), deniedDependencies);
    expect(response.status).toBe(401);
    expect(await response.json()).toEqual({ kind: "denied" });
  });

  it("sets private no-store on configured responses", async () => {
    const response = await dashboardGet(new Request(`${origin}/api/client/dashboard`, {
  ...
  it("rejects context changes from another origin before CSRF verification", async () => {
    ...
    const response = await dashboardContextPost(new Request(`${origin}/api/client/dashboard/context`, {
      method: "POST",
      headers: {
    ...
    }), { ...deniedDependencies, verifyCsrf: () => { csrfChecked = true; return true; } });
  ...
  it("accepts a same-origin context form only after CSRF and owner authorization", async () => {
    ...
    const response = await dashboardContextPost(new Request(`${origin}/api/client/dashboard/context`, {
      ...
      selectContext: async ({ requestedContext }) => requestedContext === "context-b"
    ...
    expect(response.headers.get("set-cookie")).toContain("__Host-atlas_context=opaque-selected-context");
  });
