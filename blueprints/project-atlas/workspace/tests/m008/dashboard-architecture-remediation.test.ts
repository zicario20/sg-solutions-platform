...
function activeProjection(): M007DashboardAuthProjection {
  ...
  return {
    ...
    partyLinkVersion: 3,
    organizations: [{ organizationId: "org-internal", membershipVersion: 5, entitlementVersion: 8 }],
...
describe("M008 architecture review remediation", () => {
  it("derives authorization and context choices from an active M007 server session", async () => {
    ...
    const result = await createDashboardAuthorizationSnapshot({ sessionHandle: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", locale: "en" }, port);
  ...
  it("denies inactive, expired, and revoked M007 evidence", async () => {
    ...
    for (const projection of [
      ...
      const port = createM007DashboardAuthPort(repository(projection), "0123456789abcdef0123456789abcdef");
      await expect(createDashboardAuthorizationSnapshot({ sessionHandle: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", locale: "es" }, port)).resolves.toEqual({ kind: "denied" });
    }
  });

  it("persists only a reauthorized context and denies a revoked membership", async () => {
    const port = createM007DashboardAuthPort(repository(), "0123456789abcdef0123456789abcdef");
    const first = await createDashboardAuthorizationSnapshot({ sessionHandle: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", locale: "en" }, port);
    if (first.kind !== "authorized") throw new Error("expected_authorized");
    const organization = first.snapshot.contextOptions.find((option) => option.type === "organization");
    expect(organization).toBeDefined();
    await expect(port.selectContext({ sessionHandle: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", requestedContext: organization!.opaqueRef, now: new Date() })).resolves.toMatchObject({ kind: "selected" });
    const revokedPort = createM007DashboardAuthPort(repository({ ...activeProjection(), organizations: [] }), "0123456789abcdef0123456789abcdef");
    await expect(revokedPort.selectContext({ sessionHandle: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", requestedContext: organization!.opaqueRef, now: new Date() })).resolves.toEqual({ kind: "denied" });
  });
  ...
  it("keeps manifests and workspace lockfile importers synchronized", () => {
    ...
    expect(lock).toMatch(/apps\/app:[\s\S]*?'@atlas\/dashboard':[\s\S]*?link:\.\.\/\.\.\/packages\/dashboard/u);
    expect(lock).toMatch(/apps\/app:[\s\S]*?'@atlas\/observability':[\s\S]*?link:\.\.\/\.\.\/packages\/observability/u);
  });

  it("accepts minimized public fields for every approved operational widget", () => {
    const fragment = parseDashboardFragment({
      owner: "services", snapshotId: "snapshot", sourceVersion: "services.v2", classification: "client_safe", state: "fresh", asOf: "2026-08-21T12:00:00.000Z",
      data: [{ opaqueRef: "service-opaque", title: "Tax preparation", publicState: "in_progress", statusLabel: "In progress", milestoneLabels: ["Intake", "Review"], nextStepLabel: "Upload documents", startDate: "2026-08-20", pendingTaskCount: 2, documentSummaryLabel: "1 requested", paymentSummaryLabel: "Unavailable", cta: { label: "View service", routeKey: "services" }, routeKey: "services" }],
    });
  ...
  it("uses one cookie/default locale path and renders one-language dashboard copy", () => {
    ...
    const html = renderToStaticMarkup(createElement(DashboardView, { dto: { ...dto(), locale: "en" } }));
  ...
  it("allowlists every M008 interaction and strips identifiers and payloads", () => {
    expect(DASHBOARD_EVENT_NAMES).toEqual(expect.arrayContaining([
      "client_dashboard_priority_viewed", "client_dashboard_priority_clicked", "client_dashboard_service_card_clicked", "client_dashboard_task_clicked", "client_dashboard_document_upload_clicked", "client_dashboard_appointment_clicked", "client_dashboard_payment_clicked", "client_dashboard_message_clicked", "client_dashboard_help_resource_clicked",
    ]));
    const event = recordDashboardEvent("client_dashboard_payment_clicked", { locale: "en", widgetCode: "payments", resultCode: "ok", contextId: "ctx", amount: "100", payload: "secret" });
  ...
  it("defines a disabled future cache envelope with epochs, TTL and critical freshness denial", () => {
    const authorization: DashboardAuthorizationSnapshot = { ...snapshot(), schemaVersion: "m008.auth.v2", accountStatus: "active", sessionStatus: "active", sessionExpiresAt: "2026-08-21T13:00:00.000Z", assurance: "aal1", authenticationEpoch: "7", authorizationEpoch: "11", policyEpoch: "13", contextOptions: [] };
    const envelope = buildDashboardCacheEnvelope({ snapshot: authorization, section: "help", sourceVersion: "help.v2", sourceStatus: "fresh", generatedAt: "2026-08-21T12:00:00.000Z", ttlSeconds: 60, value: { state: "fresh" } });
    expect(envelope.schemaVersion).toBe(DASHBOARD_CACHE_SCHEMA_VERSION);
    expect(envelope.expiresAt).toBe("2026-08-21T12:01:00.000Z");
    expect(canServeDashboardCacheEnvelope(envelope, authorization, "help", new Date("2026-08-21T12:00:30.000Z"))).toBe(true);
    expect(canServeDashboardCacheEnvelope({ ...envelope, section: "payments" }, authorization, "payments", new Date("2026-08-21T12:00:30.000Z"))).toBe(false);
  });
});
