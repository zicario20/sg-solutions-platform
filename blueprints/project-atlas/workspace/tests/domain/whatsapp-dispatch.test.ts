...
function fixture(
  provider:
    | { dispatch(): Promise<{ status: "accepted" }>; calls: number }
    | { dispatch(): Promise<never>; calls: number },
) {
  ...
  const service = new CommunicationsService({
    ...
    destinationResolver: {
      resolve: async () => ({ status: "resolved", endpoint: "+15555550123" }),
    },
...
async function queue(service: CommunicationsService) {
  return service.queueOutbound({
    ...
    authorizationReceipt: {
      ...
      bindingId: "binding_1",
      destinationKey: canonicalEndpointReference("endpoint_digest"),
      issuedAt: NOW,
      expiresAt: TOMORROW,
    },
...
describe("WhatsApp dispatch job", () => {
  it("records ambiguous timeout as dispatch_unknown and never blindly retries", async () => {
    ...
    ).toMatchObject({ status: "dispatch_unknown" });
    expect(
      await dispatchOutboundMessage({
        service,
        commandId,
    ...
    ).toEqual({ status: "not_dispatched", code: "dispatch_unknown_non_retryable" });
    expect(provider.calls).toBe(1);
    expect(repository.referenceState().attempts).toEqual([
      expect.objectContaining({ state: "dispatch_unknown" }),
    ]);
  });
  ...
  it("rechecks policy under the binding lock and cancels a withdrawn send before dispatch", async () => {
    ...
    await repository.withdrawContact({
      ...
      evidence: {
        source: "authority",
        receipt: {
          receiptId: "receipt_withdraw_1",
          owner: "consent",
          operation: "contact_withdrawal",
          bindingId: "binding_1",
          issuedAt: NOW,
          expiresAt: TOMORROW,
          correlationId: "correlation_withdraw_1",
        },
      },
      now: NOW,
    });
    ...
    ).toEqual({ status: "not_dispatched", code: "contact_policy_denied" });
