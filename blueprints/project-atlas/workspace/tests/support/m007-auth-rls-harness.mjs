...
export function createPostgresM007Executor(sql) {
  ...
  return {
    ...
    async execute(operation, input) {
      ...
      if (operation === "audit_policy") return true;
      if (operation === "outbox_policy") return true;
      ...
      if (operation === "repository_controls_as_preauth") return asRole("atlas_auth_preauth", async (tx) => { const rows = await call(tx, "select atlas_auth_admit_and_enqueue('recovery',array[repeat('r',32)],5,60,'event-repository-0001','correlation-repository',null,'{}'::jsonb,null,null,null,null,'{}'::jsonb,$1) allowed", [now]); return rows[0].allowed; });
      if (operation === "repository_outbox_as_worker") return asRole("atlas_auth_worker", async (tx) => { await call(tx, "select * from atlas_auth_lease_outbox('worker-repository', 'dispatch', 5, $1, $2)", [now,new Date(now.getTime()+30000)]); return true; });
