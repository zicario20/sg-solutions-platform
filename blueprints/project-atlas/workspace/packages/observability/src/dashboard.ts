...
export type DashboardEvent = Readonly<{ event: DashboardEventName; properties: Readonly<Partial<Record<"locale" | "contextType" | "widgetCode" | "actionCode" | "routeCode" | "sectionState" | "durationBucket" | "resultCode" | "policyVersion", string>>> }>;
...
const allowedValues = Object.freeze({
  ...
  widgetCode: new Set(["priority", "security", "services", "tasks", "documents", "appointments", "payments", "messages", "notifications", "help", "support"]),
  actionCode: new Set(["security_identity", "blocking_payment", "expired_document", "pending_signature", "due_task", "imminent_appointment", "missing_information", "general_action"]),
  routeCode: new Set(["home", "services", "status", "documents", "appointments", "messages", "payments", "help", "settings", "security", "support"]),
...
export function recordDashboardEvent(event: DashboardEventName, input: Readonly<Record<string, unknown>>): DashboardEvent {
