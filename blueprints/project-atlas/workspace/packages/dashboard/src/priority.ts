...
export const PRIORITY_SOURCE_REGISTRY = Object.freeze({
  ...
  documents: { highestBand: 3, allowedTypes: ["expired_document", "pending_signature", "general_action"] },
...
} satisfies Readonly<Record<string, Readonly<{ highestBand: number; allowedTypes: readonly ClientActionType[] }>>>);
...
function validateAction(source: PrioritySourceCode, action: ClientActionDto): void {
  ...
  if (!(registry.allowedTypes as readonly ClientActionType[]).includes(action.type) || !isDashboardRouteKey(action.routeKey)) throw new DashboardContractError();
  if (!action.opaqueRef || action.opaqueRef.length > 256 || !action.title || action.title.length > 256) throw new DashboardContractError();
  if (!Number.isInteger(action.workflowPriority) || !Number.isFinite(Date.parse(action.createdAt))) throw new DashboardContractError();
  if (action.dueAt !== undefined && !Number.isFinite(Date.parse(action.dueAt))) throw new DashboardContractError();
}

function validateInput(input: DashboardPriorityInput): void {
  if (!input || typeof input !== "object" || Array.isArray(input)) throw new DashboardContractError();
  const keys = Object.keys(input);
  if (keys.length !== sourceCodes.length || keys.some((key) => !sourceCodes.includes(key as PrioritySourceCode))) throw new DashboardContractError();
  for (const source of sourceCodes) {
    const value = input[source];
    if (!value || !sourceStates.has(value.state) || !Array.isArray(value.candidates) || value.candidates.length > 5) throw new DashboardContractError();
    if (value.state === "empty" && value.candidates.length !== 0) throw new DashboardContractError();
    for (const candidate of value.candidates) validateAction(source, candidate);
  }
}

const instant = (value: string | undefined): number => value === undefined ? Number.POSITIVE_INFINITY : Date.parse(value);

function compare(left: ClientActionDto, right: ClientActionDto): number {
  return bands[left.type] - bands[right.type]
...
export function selectDashboardPriority(input: DashboardPriorityInput): PriorityResult {
  validateInput(input);
  const candidates = sourceCodes.flatMap((source) => input[source].state === "fresh" ? [...input[source].candidates] : []);
  ...
  const incompleteCouldOutrank = sourceCodes.some((source) => {
    ...
    return (state === "stale" || state === "unavailable") && PRIORITY_SOURCE_REGISTRY[source].highestBand <= selectedBand;
  });
  if (incompleteCouldOutrank) return Object.freeze({ kind: "unconfirmed", safeReason: "required_source_unavailable", policyVersion: PRIORITY_POLICY_VERSION });
  if (!selected) return Object.freeze({ kind: "none", policyVersion: PRIORITY_POLICY_VERSION });
  return Object.freeze({ kind: "action", action: Object.freeze({ ...selected }), policyVersion: PRIORITY_POLICY_VERSION });
}
