...
export function resolveClientServicePublicState(
  axes: ClientServiceAxesDto,
): ClientServicePublicState {
  if (
    axes.financial === "unavailable" ||
    axes.activation === "unavailable" ||
    axes.fulfillment === "unavailable"
  )
    return "unconfirmed";
  if (axes.commercial === "preliminary" && axes.fulfillment !== "not_started") return "unconfirmed";
  if (axes.commercial === "cancelled") {
    if (axes.fulfillment !== "not_started" && axes.fulfillment !== "cancelled")
      return "unconfirmed";
    return axes.financial === "refunded" ? "refunded" : "cancelled";
  }
  if (
    ["in_progress", "waiting_client", "waiting_external", "completed"].includes(axes.fulfillment) &&
    axes.activation !== "approved" &&
    axes.activation !== "not_required"
  )
    return "unconfirmed";
  ...
  if (axes.fulfillment === "in_progress")
    return axes.activation === "approved" || axes.activation === "not_required"
      ? "in_progress"
      : "unconfirmed";
  ...
  if (axes.activation === "approved" || axes.activation === "not_required") {
    ...
    if (
      axes.financial === "paid" &&
      axes.commercial === "active" &&
      axes.fulfillment === "not_started"
    )
