...
export class PostgresAuthSessionInvitationRepository {
  ...
  async issue(input: InvitationIssue): Promise<void> {
    await this.sql.begin((transaction) =>
      query(transaction, "select atlas_auth_issue_invitation($1,$2,$3,$4,$5,$6,$7,$8)", [
        input.id,
        input.proofDigest,
        input.contactId,
        input.scope,
        input.inviterAccountId,
        input.expectedProviderSubject,
        input.expiresAt,
        input.now,
      ]),
    );
  }
  async createInvitation(input: InvitationIssue): Promise<void> {
  ...
  async consume(input: InvitationConsume): Promise<{ kind: "consumed" | "manual_review" }> {
    const rows = await this.sql.begin((transaction) =>
      query<readonly { outcome: "consumed" | "manual_review" }[]>(
        transaction,
        "select atlas_auth_consume_invitation($1,$2,$3,$4) as outcome",
        [input.id, input.proofDigest, input.sessionHandleDigest, input.now],
      ),
    );
    return { kind: rows[0]?.outcome === "consumed" ? "consumed" : "manual_review" };
  }
  async acceptInvitation(
    input: InvitationConsume,
  ...
  async createSession(input: {
    ...
    await this.sql.begin((transaction) =>
      query(transaction, "select atlas_auth_create_session($1,$2,$3,$4,$5,$6,$7,$8)", [
        input.id,
        input.accountId,
        input.handleDigest,
        input.familyId,
  ...
  async listAndTouchSessions(
    handleDigest: string,
    now: Date,
  ): Promise<readonly { id: string; created_at: Date; is_current: boolean }[]> {
    return this.sql.begin((transaction) =>
      query(transaction, "select * from atlas_auth_list_sessions($1,$2)", [handleDigest, now]),
    );
  }
}
