...
export class PostgresAuthControlPlaneRepository {
  ...
  async rotateSession(input: { handleDigest: string; next: SessionRotationInsert; now: Date }): Promise<"rotated" | "family_revoked"> {
    const rows = await this.sql.begin((transaction) => query<readonly { outcome: "rotated" | "family_revoked" }[]>(transaction, "select atlas_auth_rotate_session($1,$2,$3,$4,$5) as outcome", [input.handleDigest,input.next.id,input.next.handleDigest,input.next.idleExpiresAt,input.now]));
