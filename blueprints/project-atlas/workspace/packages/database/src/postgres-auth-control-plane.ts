...
type SessionRotationInsert = { id: string; handleDigest: string; idleExpiresAt: Date };
const query = <T>(
...
export class PostgresAuthControlPlaneRepository {
  ...
  async rotateSession(input: {
    handleDigest: string;
    next: SessionRotationInsert;
    now: Date;
  }): Promise<"rotated" | "family_revoked"> {
    const rows = await this.sql.begin((transaction) =>
      query<readonly { outcome: "rotated" | "family_revoked" }[]>(
        transaction,
        "select atlas_auth_rotate_session($1,$2,$3,$4,$5) as outcome",
        [
          input.handleDigest,
          input.next.id,
          input.next.handleDigest,
          input.next.idleExpiresAt,
          input.now,
        ],
      ),
    );
  ...
  async admit(input: {
    ...
    const rows = await this.sql.begin((transaction) =>
      query<readonly { allowed: boolean }[]>(
        transaction,
        "select atlas_auth_admit_pre_auth($1,$2,$3,$4) as allowed",
        [input.bucketDigest, input.purpose, input.commandId, input.now],
      ),
    );
  ...
  async revokeByHandleDigest(handleDigest: string, now: Date): Promise<boolean> {
    const rows = await this.sql.begin((transaction) =>
      query<readonly { revoked: boolean }[]>(
        transaction,
        "select atlas_auth_revoke_current($1,$2) as revoked",
        [handleDigest, now],
      ),
    );
  ...
  async revokeOthersByHandleDigest(handleDigest: string, now: Date): Promise<boolean> {
    const rows = await this.sql.begin((transaction) =>
      query<readonly { revoked: boolean }[]>(
        transaction,
        "select atlas_auth_revoke_others($1,$2) as revoked",
        [handleDigest, now],
      ),
    );
    return rows[0]?.revoked === true;
