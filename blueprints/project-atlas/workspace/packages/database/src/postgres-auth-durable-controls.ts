...
export class PostgresDurableAuthControlsRepository {
  ...
  async lease(input: { readonly owner: string; readonly leasePurpose: "dispatch" | "reconcile"; readonly limit: number; readonly now: Date; readonly leaseExpiresAt: Date }) {
    const rows = await this.sql.begin((transaction) => query<readonly { command_id: string; purpose: string; channel: OutboxChannel; idempotency_key: string; payload: Readonly<Record<string, unknown>>; attempt_count: number; lease_version: number; lease_owner: string; lease_purpose: "dispatch" | "reconcile" }[]>(transaction, "select * from atlas_auth_lease_outbox($1,$2,$3,$4,$5)", [input.owner, input.leasePurpose, input.limit, input.now, input.leaseExpiresAt]));
    return rows.map((row) => ({ commandId: row.command_id, purpose: row.purpose, channel: row.channel, idempotencyKey: row.idempotency_key, payload: row.payload, attemptCount: row.attempt_count, leaseVersion: row.lease_version, leaseOwner: row.lease_owner, leasePurpose: row.lease_purpose }));
  }
