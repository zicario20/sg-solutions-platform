...
const auditOutcomeKeys = new Set([
  "accepted",
  "succeeded",
  "authenticated",
  "denied",
...
function safeAuditMetadata(metadata: AuditMetadata): AuditMetadata {
  for (const [key, value] of Object.entries(metadata)) {
    if (!auditMetadataKeys.has(key) || !["string", "number", "boolean"].includes(typeof value))
      throw new Error("AUTH_AUDIT_METADATA_FIELD_DENIED");
    if (
      typeof value === "string" &&
      (value.length > 128 ||
        /[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}|bearer\s|password|secret|api[_-]?key|eyJ[A-Za-z0-9_-]{10,}/iu.test(
          value,
        ))
    )
      throw new Error("AUTH_AUDIT_METADATA_VALUE_DENIED");
  }
  return metadata;
}
...
export class PostgresDurableAuthControlsRepository {
  ...
  async appendAudit(input: AuditInput): Promise<{ readonly kind: "appended" | "duplicate" }> {
    ...
    const rows = await this.sql.begin((transaction) =>
      query<readonly { appended: boolean }[]>(
        transaction,
        "select atlas_auth_append_audit($1,$2,$3,$4,$5,$6::jsonb,$7) as appended",
        [
          input.eventKey,
          input.eventName,
          input.outcome,
  ...
  async admitAndEnqueue(
    ...
    if (
      !/^[a-z_]{3,64}$/u.test(input.action) ||
      input.riskKeyDigests.length < 1 ||
      input.riskKeyDigests.length > 5 ||
      input.riskKeyDigests.some((key) => key.length < 16) ||
      !Number.isSafeInteger(input.threshold) ||
      input.threshold < 1 ||
      !Number.isSafeInteger(input.windowSeconds) ||
      input.windowSeconds < 1
    )
      throw new Error("AUTH_RATE_INPUT_DENIED");
    const rows = await this.sql.begin((transaction) =>
      query<readonly { allowed: boolean }[]>(
        transaction,
        "select atlas_auth_admit_and_enqueue($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9,$10,$11,$12,$13::jsonb,$14) as allowed",
        [
          input.action,
          input.riskKeyDigests,
          input.threshold,
          input.windowSeconds,
          input.eventKey,
          input.correlationId,
          input.accountId ?? null,
          JSON.stringify(safeAuditMetadata(input.metadata)),
          input.outbox?.commandId ?? null,
          input.outbox?.purpose ?? null,
          input.outbox?.channel ?? null,
  ...
  async recoverExpiredLeases(
    ...
    const rows = await this.sql.begin((transaction) =>
      query<readonly { dispatch_to_reconcile: number; reconcile_to_manual_review: number }[]>(
        transaction,
        "select * from atlas_auth_recover_outbox_leases($1)",
        [now],
      ),
    );
    return {
      dispatchToReconcile: rows[0]?.dispatch_to_reconcile ?? 0,
      reconcileToManualReview: rows[0]?.reconcile_to_manual_review ?? 0,
    };
  }
  async lease(input: {
    ...
    const rows = await this.sql.begin((transaction) =>
      ...
      >(transaction, "select * from atlas_auth_lease_outbox($1,$2,$3,$4,$5)", [
        input.owner,
        input.leasePurpose,
        input.limit,
        input.now,
  ...
  async recordDispatchOutcome(input: {
    ...
    await this.sql.begin((transaction) =>
      query(transaction, "select atlas_auth_record_dispatch_outcome($1,$2,$3,$4,$5,$6,$7)", [
        input.commandId,
        input.owner,
        input.leaseVersion,
        input.outcome,
  ...
  async recordReconciliation(input: {
    ...
    await this.sql.begin((transaction) =>
      query(transaction, "select atlas_auth_record_reconciliation($1,$2,$3,$4,$5,$6,$7)", [
        input.commandId,
        input.owner,
        input.leaseVersion,
        input.outcome,
