...
export const voiceCommandReceipts = pgTable(
  ...
  (table) => [
    ...
    check("voice_command_receipts_result_valid", sql`(${table.state} = 'reserved' and ${table.resultKind} is null and ${table.resultCode} is null and ${table.ownerReceiptId} is null and ${table.completedAt} is null) or (${table.state} in ('completed', 'failed') and ${table.resultKind} is not null and ${table.completedAt} >= ${table.issuedAt} and ((${table.resultKind} = 'completed' and ${table.resultCode} is not null and ${table.ownerReceiptId} is not null) or (${table.resultKind} <> 'completed' and ${table.resultCode} is null and ${table.ownerReceiptId} is null)))`),
