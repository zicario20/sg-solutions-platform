...
export const voiceCommandReceipts = pgTable(
  ...
  (table) => [
    ...
    check("voice_command_receipts_state_valid", sql`${table.state} in ('reserved', 'reconciliation_required', 'completed', 'failed')`),
    ...
    check("voice_command_receipts_result_kind_valid", sql`${table.resultKind} is null or ${table.resultKind} in ('completed', 'verification_required', 'confirmation_required', 'denied', 'unavailable')`),
    check("voice_command_receipts_result_code_valid", sql`${table.resultCode} is null or ${table.resultCode} in ('language_selected', 'contact_hint_processed', 'public_information_ready', 'availability_ready', 'lead_created', 'appointment_requested', 'callback_requested', 'message_recorded', 'transfer_requested', 'voicemail_requested', 'approved_link_requested', 'portal_required', 'safe_status_ready', 'payment_projection_ready', 'missing_documents_ready', 'next_appointment_ready', 'secure_message_recorded')`),
    check("voice_command_receipts_result_valid", sql`(${table.state} in ('reserved', 'reconciliation_required') and ${table.resultKind} is null and ${table.resultCode} is null and ${table.ownerReceiptId} is null and ${table.completedAt} is null) or (${table.state} in ('completed', 'failed') and ${table.resultKind} is not null and ${table.completedAt} >= ${table.issuedAt} and ((${table.resultKind} = 'completed' and ${table.resultCode} is not null and ${table.ownerReceiptId} is not null) or (${table.resultKind} <> 'completed' and ${table.resultCode} is null and ${table.ownerReceiptId} is null)))`),
