...
}, (table) => [uniqueIndex("lesson_records_code_unique").on(table.code), index("lesson_records_status_idx").on(table.status)]);

export const lessonEvidenceLinks = pgTable("lesson_evidence_links", {
...
}, (table) => [uniqueIndex("lesson_evidence_links_code_unique").on(table.code), index("lesson_evidence_links_record_idx").on(table.recordCode)]);

export const lessonReviewRequests = pgTable("lesson_review_requests", {
...
}, (table) => [uniqueIndex("lesson_review_requests_code_unique").on(table.code), index("lesson_review_requests_record_idx").on(table.recordCode)]);

export const lessonActionRequests = pgTable("lesson_action_requests", {
...
}, (table) => [uniqueIndex("lesson_action_requests_code_unique").on(table.code), index("lesson_action_requests_record_idx").on(table.recordCode)]);
