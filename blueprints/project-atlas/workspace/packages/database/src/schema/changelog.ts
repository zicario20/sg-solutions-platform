...
export const changelogRecords = pgTable("changelog_records", {
...
}, (table) => [uniqueIndex("changelog_records_code_unique").on(table.code), index("changelog_records_status_idx").on(table.status)]);

export const changelogEvidenceLinks = pgTable("changelog_evidence_links", {
...
}, (table) => [uniqueIndex("changelog_evidence_links_code_unique").on(table.code), index("changelog_evidence_links_record_idx").on(table.recordCode)]);

export const changelogReviewRequests = pgTable("changelog_review_requests", {
...
}, (table) => [uniqueIndex("changelog_review_requests_code_unique").on(table.code), index("changelog_review_requests_record_idx").on(table.recordCode)]);

export const changelogActionRequests = pgTable("changelog_action_requests", {
...
}, (table) => [uniqueIndex("changelog_action_requests_code_unique").on(table.code), index("changelog_action_requests_record_idx").on(table.recordCode)]);
