...
export type InternalEmailAuthOutcome = "accepted" | "succeeded" | "denied" | "provider_denied" | "provider_unavailable" | "provider_error" | "provider_exception";
...
const neutral = (internalOutcome: InternalEmailAuthOutcome) => ({ kind: "accepted" as const, internalOutcome });
...
export function createServerEmailAuthService(options: { provider: ServerEmailAuthProvider; repository: EmailAuthRepository; sealProviderToken(value: string): string; openProviderToken?(value: string): string }, now = () => new Date()) {
  const establish = async (verified: VerifiedEmailAuthority) => {
    ...
    } catch { return neutral("provider_error"); }
  };
  return {
    async signUp(input: { email: string; password: string }) { try { return neutral(providerOutcome(await options.provider.signUp(input))); } catch { return neutral("provider_exception"); } },
    async sendVerification(input: { email: string }) { try { return neutral(providerOutcome(await options.provider.sendVerification(input))); } catch { return neutral("provider_exception"); } },
    async requestRecovery(input: { email: string }) { try { return neutral(providerOutcome(await options.provider.requestRecovery(input))); } catch { return neutral("provider_exception"); } },
    async signIn(input: { email: string; password: string }) { let result: VerifiedEmailAuthority | NeutralProviderResult; try { result = await options.provider.signIn(input); } catch { return neutral("provider_exception"); } return authority(result) ? establish(result) : neutral(providerOutcome(result)); },
    async consumeVerification(input: { token: string }) { let result: VerifiedEmailAuthority | NeutralProviderResult; try { result = await options.provider.consumeVerification(input); } catch { return neutral("provider_exception"); } if (!authority(result)) return neutral(providerOutcome(result)); try { if (!(await options.repository.consumeProviderToken({ tokenDigest: digestOpaqueProof(input.token), purpose: "verification", now: now() }))) return neutral("denied"); } catch { return neutral("provider_error"); } return establish(result); },
    async consumeReset(input: { token: string; password: string }) { let result: VerifiedEmailAuthority | NeutralProviderResult; try { result = await options.provider.consumeRecovery({ token: input.token }); } catch { return neutral("provider_exception"); } if (!authority(result)) return neutral(providerOutcome(result)); try { if (!(await options.repository.consumeProviderToken({ tokenDigest: digestOpaqueProof(input.token), purpose: "recovery", now: now() }))) return neutral("denied"); } catch { return neutral("provider_error"); } let changed: NeutralProviderResult; try { changed = await options.provider.updatePassword({ accessToken: result.accessToken, password: input.password }); } catch { return neutral("provider_exception"); } return changed.kind === "accepted" ? establish(result) : neutral(providerOutcome(changed)); },
