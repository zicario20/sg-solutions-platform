...
export type DurableInvitationRepository = Readonly<{ issue(input: { id: string; proofDigest: string; contactId: string; scope: string; inviterAccountId: string; expectedProviderSubject: string; expiresAt: Date; now: Date }): Promise<void>; consume(input: { id: string; proofDigest: string; sessionHandleDigest: string; now: Date }): Promise<{ kind: "consumed" | "manual_review" }> }>;
export function createDurableInvitationService(repository: DurableInvitationRepository, now = () => new Date()) {
  return { async issue(input: { contactId: string; scope: string; inviterAccountId: string; expectedProviderSubject: string }) { const id = createOpaqueValue(); const proof = createOpaqueValue(); const issued = now(); await repository.issue({ id, proofDigest: digestOpaqueProof(proof), ...input, expiresAt: new Date(issued.getTime() + 15 * 60_000), now: issued }); return { id, proof }; }, async consume(input: { id: string; proof: string; sessionHandle: string }) { return repository.consume({ id: input.id, proofDigest: digestOpaqueProof(input.proof), sessionHandleDigest: digestOpaqueProof(input.sessionHandle), now: now() }); } };
}
