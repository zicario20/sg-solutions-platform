...
export function createDurableOAuthTransactionService(
  repository: DurableOAuthTransactionRepository,
  now = () => new Date(),
) {
  return {
    async begin(input: {
      ...
      if (
        !/^https:\/\/[^/?#]+\/api\/auth\/oauth\/google\/callback$/u.test(input.callbackUrl) ||
        !/^\/(?!\/)[^\r\n]*$/u.test(input.returnIntent)
      )
        throw new Error("OAUTH_RETURN_INTENT_DENIED");
      ...
      await repository.issue({
        ...
        redirectHash: digestOpaqueProof(input.callbackUrl),
    ...
    async consume(input: {
      ...
      return repository.consume({
        ...
        nonceDigest: digestOpaqueProof(input.nonce),
        pkceVerifierDigest: digestOpaqueProof(input.pkceVerifier),
        browserBindingDigest: digestOpaqueProof(input.browserBinding),
        redirectHash: digestOpaqueProof(input.callbackUrl),
        now: now(),
      });
