...
export type AuthProviderOutcome = Readonly<{ readonly outcome: "sent" | "failed" | "unknown"; readonly providerMessageId?: string; readonly errorCode?: string }>;
...
export type DurableAuthOutboxRepository = Readonly<{
  ...
  lease(input: { readonly owner: string; readonly leasePurpose: "dispatch" | "reconcile"; readonly limit: number; readonly now: Date; readonly leaseExpiresAt: Date }): Promise<readonly DurableAuthOutboxCommand[]>;
  recordDispatchOutcome(input: { readonly commandId: string; readonly owner: string; readonly leaseVersion: number; readonly outcome: "sent" | "failed" | "unknown"; readonly providerMessageId?: string; readonly errorCode?: string; readonly now: Date }): Promise<void>;
  recordReconciliation(input: { readonly commandId: string; readonly owner: string; readonly leaseVersion: number; readonly outcome: "sent" | "failed" | "unknown"; readonly providerMessageId?: string; readonly errorCode?: string; readonly now: Date }): Promise<void>;
}>;
export type DurableAuthDeliveryProvider = Readonly<{
  ...
  queryByOwner(command: DurableAuthOutboxCommand, signal: AbortSignal): Promise<AuthProviderOutcome>;
}>;

async function withWorkerTimeout<T>(timeoutMs: number, operation: (signal: AbortSignal) => Promise<T>): Promise<T> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  try {
    return await Promise.race([
      operation(controller.signal),
      new Promise<T>((_resolve, reject) => controller.signal.addEventListener("abort", () => reject(new Error("AUTH_OUTBOX_PROVIDER_TIMEOUT")), { once: true })),
    ]);
  } finally {
...
export function createDurableAuthOutboxWorker(options: {
  ...
  if (!options.owner || !Number.isSafeInteger(options.maxJobs) || options.maxJobs < 1 || options.maxJobs > 100 || !Number.isSafeInteger(options.timeoutMs) || options.timeoutMs < 100 || options.timeoutMs > 60_000) throw new Error("AUTH_OUTBOX_WORKER_CONFIG_INVALID");
  const clock = options.now ?? (() => new Date());
  return {
    async run(): Promise<{ readonly kind: "processed" | "unavailable"; readonly processed: number }> {
      ...
      for (const command of commands.slice(0, options.maxJobs)) {
        ...
        const common = { commandId: command.commandId, owner: options.owner, leaseVersion: command.leaseVersion, ...result, now: clock() };
        if (options.leasePurpose === "dispatch") await options.repository.recordDispatchOutcome(common);
