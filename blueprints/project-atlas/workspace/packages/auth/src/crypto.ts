...
const serverSecretKey = (secret: string) => { assertNonEmpty(secret, "server_secret"); return createHash("sha256").update(secret, "utf8").digest(); };
export function sealServerSecret(secret: string, value: string): string {
  assertNonEmpty(value, "secret_value"); const iv = randomBytes(12); const cipher = createCipheriv("aes-256-gcm", serverSecretKey(secret), iv); const ciphertext = Buffer.concat([cipher.update(value, "utf8"), cipher.final()]);
  return `v1.${iv.toString("base64url")}.${cipher.getAuthTag().toString("base64url")}.${ciphertext.toString("base64url")}`;
}
export function openServerSecret(secret: string, sealed: string): string {
  const [version, iv, tag, ciphertext] = sealed.split("."); if (version !== "v1" || !iv || !tag || !ciphertext) throw new Error("sealed_secret_denied");
  const decipher = createDecipheriv("aes-256-gcm", serverSecretKey(secret), Buffer.from(iv, "base64url")); decipher.setAuthTag(Buffer.from(tag, "base64url")); return Buffer.concat([decipher.update(Buffer.from(ciphertext, "base64url")), decipher.final()]).toString("utf8");
}
