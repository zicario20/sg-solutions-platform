...
export function createDurableSessionService(repository: DurableSessionRepository, now = () => new Date()) {
  ...
  return {
    ...
    async rotate(input: { handle: string }) { const issued = now(); const handle = createOpaqueValue(); const result = await repository.rotate({ handleDigest: digestOpaqueProof(input.handle), next: { handleDigest: digestOpaqueProof(handle), idleExpiresAt: expiry(issued).idleExpiresAt }, now: issued }); return result === "rotated" ? { kind: "rotated" as const, handle } : { kind: "family_revoked" as const }; },
