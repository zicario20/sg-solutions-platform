...
export function createAuthRuntime(input: { canonicalOrigin: string; sessionStore?: unknown; config?: AuthRuntimeConfig }): AuthRuntime {
  ...
  return {
    ...
    async execute(command, request) {
      ...
      if (command === "register" || command === "recovery" || command === "verify") return { status: 202, body: { kind: "accepted" } };
