...
export type AuthRuntime = {
  ...
  ): Promise<{ status: number; body: { kind: "accepted" | "denied" | "unavailable" } }>;
};
...
export function createAuthRuntime(input: {
  ...
  controls?: DurableAuthControls;
}): AuthRuntime {
  if (!/^https:\/\/[^/?#]+$/u.test(input.canonicalOrigin))
    throw new Error("auth_canonical_origin_invalid");
  return {
    canonicalOrigin: input.canonicalOrigin,
    async execute(command, request) {
      if (request.origin !== input.canonicalOrigin)
        return { status: 403, body: { kind: "denied" } };
      ...
      if (
        ...
        const admission = await createTransactionalAuthControls(input.controls).admit({
          purpose: command,
          identifierDigest: "server_derived_request",
        });
        return admission.kind === "accepted"
          ? { status: 202, body: { kind: "accepted" } }
          : { status: 503, body: { kind: "unavailable" } };
      }
