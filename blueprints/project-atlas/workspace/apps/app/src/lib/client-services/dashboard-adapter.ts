...
function dashboardFragment(result: ClientServicesQueryResult<ClientServiceListDto>) {
  ...
  return {
    ...
    classification: "client_safe" as const,
    generatedAt: new Date(),
    data: result.dto.items.slice(0, 4).map((item) => ({
      opaqueRef: item.opaqueRef,
      title: item.serviceName,
      statusLabel: item.publicStateLabel,
      publicState: item.publicState,
      nextStepLabel: item.nextStepLabel,
      cta: { routeKey: "services" as const },
    })),
  };
}
...
export async function loadAuthorizedClientServicesDashboardFragment(
  query: AuthorizedListQuery,
  snapshot: DashboardAuthorizationSnapshot,
  limit: number,
) {
  return dashboardFragment(await query.listAuthorized({ snapshot, limit }));
}

export function createClientServicesDashboardOwnerPort(
  load: (input: {
    snapshot: DashboardAuthorizationSnapshot;
    signal: AbortSignal;
    limit: number;
  }) => ReturnType<typeof loadClientServicesDashboardFragment>,
) {
  return Object.freeze({
    owner: "services" as const,
    query: async ({
      snapshot,
      snapshotId,
      signal,
      ...
      readonly snapshot: DashboardAuthorizationSnapshot;
      ...
      return Object.freeze({
        owner: "services" as const,
        snapshotId,
        sourceVersion: "m009.service-order.v2",
        ...fragment,
      });
    },
  });
