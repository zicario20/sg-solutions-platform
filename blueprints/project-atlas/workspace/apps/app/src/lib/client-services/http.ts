...
import {
  type ClientServicesAnalyticsEvent,
  createClientServicesAnalyticsEvent,
...
async function admitted(
  action: ClientServicesAdmissionAction,
  request: Request,
  dependencies: ClientServicesHttpDependencies,
): Promise<Response | undefined> {
  try {
    return (await dependencies.admit(action, request))
      ? undefined
...
export async function handleClientServicesListGet(
  request: Request,
  dependencies: ClientServicesHttpDependencies,
): Promise<Response> {
  ...
  if (
    ...
    (url.searchParams.get("status") && (!status || !allowedStatuses.includes(status))) ||
    url.searchParams.has("cursor")
  )
    return json(400, { error: "invalid_request" });
  const queried = await dependencies.query.list({ request, query, status, limit: 24 });
  const value =
    queried.kind === "ok" && status
      ? {
          kind: "ok" as const,
          dto: {
            ...queried.dto,
            items: queried.dto.items.filter((item) => item.publicState === status),
          },
        }
      : queried;
  const outcome =
    value.kind === "ok"
      ? value.dto.items.length
        ? "available"
        : "empty"
  ...
  try {
    await dependencies.emitAnalytics?.(
      createClientServicesAnalyticsEvent(
        query || status ? "client_services_health_filtered" : "client_services_health_list",
        { outcome, filterUsed: Boolean(query || status) },
      ),
    );
  } catch {
    /* operational health metrics never change the response */
  }
  return result(value);
}

export async function handleClientServiceDetailGet(
  request: Request,
  opaqueRef: string,
  dependencies: ClientServicesHttpDependencies,
): Promise<Response> {
