...
import { createServerAuthRuntime, type AuthAuditOutcome, type AuthAuditPurpose, type ServerAuthControlPlane } from "./server-runtime.ts";
...
type ActiveAuthSession = { readonly id: string; readonly createdAt?: Date; readonly current?: boolean };
type DurableHttpAdapter = { sessions: { list(handle: string): Promise<readonly ActiveAuthSession[]>; rotate(handle: string): Promise<{ kind: "rotated"; handle: string } | { kind: "family_revoked" }>; revokeCurrent(handle: string): Promise<{ kind: "revoked" | "denied" }>; revokeOthers(handle: string): Promise<{ kind: "revoked" | "denied" }> }; invitations: { accept(input: { id: string; proof: string; sessionHandle: string }): Promise<{ kind: "consumed" | "manual_review" }> } };
type DurableHttpSecurity = { readonly canonicalOrigin: string; readonly auditOutcome?: (input: { readonly purpose: AuthAuditPurpose; readonly outcome: AuthAuditOutcome; readonly requestId: string; readonly now: Date }) => Promise<void> };
...
const csrf = (request: Request, handle: string, secret: string) => { const value = requestCookie(request, "__Host-atlas_csrf"); return Boolean(value && value === request.headers.get("x-atlas-csrf") && verifySessionCsrfToken(secret, handle, value)); };

export function createDurableAuthHttpFactory(adapter: DurableHttpAdapter, csrfSecret = process.env.AUTH_SESSION_CSRF_SECRET ?? "", security: DurableHttpSecurity = { canonicalOrigin: process.env.AUTH_CANONICAL_ORIGIN ?? "" }) {
  const respond = (status: number, body?: Record<string, unknown>) => body ? Response.json(body, { status, headers: { "cache-control": "private, no-store", "referrer-policy": "no-referrer" } }) : new Response(null, { status, headers: { "cache-control": "private, no-store", "referrer-policy": "no-referrer" } });
  const audit = async (purpose: AuthAuditPurpose, outcome: AuthAuditOutcome, request: Request) => security.auditOutcome?.({ purpose, outcome, requestId: request.headers.get("x-request-id") ?? crypto.randomUUID(), now: new Date() });
  const mutationHandle = async (request: Request) => {
    if (request.method !== "POST" || !security.canonicalOrigin || new URL(request.url).origin !== security.canonicalOrigin || request.headers.get("origin") !== security.canonicalOrigin) return undefined;
    const handle = requestCookie(request, "__Host-atlas_auth"); const csrfCookie = requestCookie(request, "__Host-atlas_csrf");
    const form = await request.clone().formData().catch(() => new FormData()); const csrfRequest = request.headers.get("x-atlas-csrf") ?? String(form.get("csrf") ?? "");
    return handle && csrfCookie && csrfRequest && csrfCookie === csrfRequest && verifySessionCsrfToken(csrfSecret, handle, csrfRequest) ? handle : undefined;
  };
  return {
    sessions: async (request: Request) => { const handle = requestCookie(request, "__Host-atlas_auth"); if (!handle || !csrf(request, handle, csrfSecret)) return Response.json({ kind: "denied" }, { status: 403 }); const sessions = (await adapter.sessions.list(handle)).slice(0, 20).map((session, index) => ({ displayRef: `session-${index + 1}`, createdAt: session.createdAt, current: session.current === true })); return Response.json({ sessions }, { status: 200, headers: { "cache-control": "private, no-store" } }); },
    rotate: async (request: Request) => { const handle = await mutationHandle(request); if (!handle) { await audit("sessions", "denied", request); return respond(403, { kind: "denied" }); } const result = await adapter.sessions.rotate(handle); await audit("sessions", result.kind === "rotated" ? "rotated" : "denied", request); if (result.kind !== "rotated") return respond(403, { kind: "denied" }); const headers = new Headers({ "cache-control": "private, no-store", "referrer-policy": "no-referrer" }); for (const value of createSessionCookieHeaders(result.handle, csrfSecret)) headers.append("set-cookie", value); return new Response(null, { status: 204, headers }); },
    acceptInvitation: async (request: Request) => { const handle = await mutationHandle(request); if (!handle) { await audit("invitation_accept", "denied", request); return respond(403, { kind: "denied" }); } const form = await request.formData(); const result = await adapter.invitations.accept({ id: String(form.get("id") ?? ""), proof: String(form.get("code") ?? ""), sessionHandle: handle }); const outcome = result.kind === "consumed" ? "accepted" : "manual_review"; await audit("invitation_accept", outcome, request); return respond(202, { kind: outcome }); },
  };
}
  ...
  return {
    ...
    callback: async (request: Request) => {
      ...
      if (result.kind === "authenticated") {
        ...
        for (const value of createSessionCookieHeaders(result.handle, csrfSecret)) headers.append("set-cookie", value);
...
function configuredDurableAdapter(): DurableHttpAdapter | undefined {
  ...
  return { sessions: { list: async (handle) => (await durable.listAndTouchSessions(canonicalSessionHandleDigest(handle), new Date())).map((row) => ({ id: row.id, createdAt: row.created_at, current: row.is_current })), rotate: async (handle) => { const now = new Date(); const next = createOpaqueValue(); const result = await sessions.rotateSession({ handleDigest: canonicalSessionHandleDigest(handle), next: { id: createOpaqueValue(), handleDigest: canonicalSessionHandleDigest(next), idleExpiresAt: new Date(now.getTime() + 30 * 60_000) }, now }); return result === "rotated" ? { kind: "rotated" as const, handle: next } : { kind: "family_revoked" as const }; }, revokeCurrent: async (handle) => ({ kind: await sessions.revokeByHandleDigest(canonicalSessionHandleDigest(handle), new Date()) ? "revoked" as const : "denied" as const }), revokeOthers: async (handle) => ({ kind: await sessions.revokeOthersByHandleDigest(canonicalSessionHandleDigest(handle), new Date()) ? "revoked" as const : "denied" as const }) }, invitations: { accept: async (input) => durable.consume({ id: input.id, proofDigest: digestOpaqueProof(input.proof), sessionHandleDigest: canonicalSessionHandleDigest(input.sessionHandle), now: new Date() }) } };
}

export function createAuthSessionPageLoader(loadAdapter: () => Promise<{ readonly sessions: { list(handle: string): Promise<readonly ActiveAuthSession[]> } } | undefined> | { readonly sessions: { list(handle: string): Promise<readonly ActiveAuthSession[]> } } | undefined) {
  return async (handle: string): Promise<readonly ActiveAuthSession[]> => { if (!handle) return []; try { return (await loadAdapter())?.sessions.list(handle) ?? []; } catch { return []; } };
}
...
function configuredOAuthAdapter(): DefaultOAuthAdapter {
  return createDefaultOAuthAdapter(process.env, createConfiguredOAuthDependencies(process.env) ?? {});
}

export function createAuthEntryPoints(loadAdapter: () => Promise<DurableHttpAdapter | undefined> | DurableHttpAdapter | undefined, csrfSecret = process.env.AUTH_SESSION_CSRF_SECRET ?? "", canonicalOrigin = process.env.AUTH_CANONICAL_ORIGIN ?? "", controlPlane: ServerAuthControlPlane | undefined = configuredControlPlane()) {
  const durableSecurity = { canonicalOrigin, auditOutcome: controlPlane?.auditOutcome };
  return { get: async (request: Request) => { if (new URL(request.url).pathname.endsWith("/sessions")) { const adapter = await loadAdapter(); return adapter ? createDurableAuthHttpFactory(adapter, csrfSecret, durableSecurity).sessions(request) : Response.json({ kind: "unavailable" }, { status: 503 }); } return authGet(request); }, post: async (request: Request) => { const url = new URL(request.url); if (url.pathname.endsWith("/sessions") && request.headers.get("x-atlas-session-action") === "rotate") { const adapter = await loadAdapter(); return adapter ? createDurableAuthHttpFactory(adapter, csrfSecret, durableSecurity).rotate(request) : Response.json({ kind: "unavailable" }, { status: 503 }); } if (url.pathname.includes("/invitations/accept")) { const adapter = await loadAdapter(); return adapter ? createDurableAuthHttpFactory(adapter, csrfSecret, durableSecurity).acceptInvitation(request) : Response.json({ kind: "unavailable" }, { status: 503 }); } return authPost(request); } };
}
