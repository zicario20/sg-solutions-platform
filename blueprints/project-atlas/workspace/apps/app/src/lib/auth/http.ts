...
export function createDurableAuthHttpFactory(adapter: DurableHttpAdapter, csrfSecret = process.env.AUTH_SESSION_CSRF_SECRET ?? "", security: DurableHttpSecurity = { canonicalOrigin: process.env.AUTH_CANONICAL_ORIGIN ?? "" }) {
  ...
  return {
    sessions: async (request: Request) => { const handle = requestCookie(request, "__Host-atlas_auth"); if (!handle || !csrf(request, handle, csrfSecret)) return Response.json({ kind: "denied" }, { status: 403 }); return Response.json({ sessions: await adapter.sessions.list(handle) }, { status: 200, headers: { "cache-control": "private, no-store" } }); },
