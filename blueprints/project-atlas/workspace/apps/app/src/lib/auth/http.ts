...
export function createDurableAuthHttpFactory(adapter: DurableHttpAdapter, csrfSecret = process.env.AUTH_SESSION_CSRF_SECRET ?? "") {
  return {
    ...
    rotate: async (request: Request) => { const handle = requestCookie(request, "__Host-atlas_auth"); if (!handle || !csrf(request, handle, csrfSecret)) return Response.json({ kind: "denied" }, { status: 403 }); const result = await adapter.sessions.rotate(handle); if (result.kind !== "rotated") return Response.json({ kind: "denied" }, { status: 403 }); const headers = new Headers(); for (const value of createSessionCookieHeaders(result.handle, csrfSecret)) headers.append("set-cookie", value); return new Response(null, { status: 204, headers }); },
    acceptInvitation: async (request: Request) => { const handle = requestCookie(request, "__Host-atlas_auth"); if (!handle) return Response.json({ kind: "denied" }, { status: 403 }); const form = await request.formData(); const result = await adapter.invitations.accept({ id: String(form.get("id") ?? ""), proof: String(form.get("proof") ?? ""), sessionHandle: handle }); return Response.json({ kind: result.kind === "consumed" ? "accepted" : "manual_review" }, { status: 202 }); },
  };
}
...
export function createAuthEntryPoints(loadAdapter: () => Promise<DurableHttpAdapter | undefined> | DurableHttpAdapter | undefined, csrfSecret = process.env.AUTH_SESSION_CSRF_SECRET ?? "") {
  return { get: async (request: Request) => { if (new URL(request.url).pathname.endsWith("/sessions")) { const adapter = await loadAdapter(); return adapter ? createDurableAuthHttpFactory(adapter, csrfSecret).sessions(request) : Response.json({ kind: "unavailable" }, { status: 503 }); } return authGet(request); }, post: async (request: Request) => { const url = new URL(request.url); if (url.pathname.endsWith("/sessions") && request.headers.get("x-atlas-session-action") === "rotate") { const adapter = await loadAdapter(); return adapter ? createDurableAuthHttpFactory(adapter, csrfSecret).rotate(request) : Response.json({ kind: "unavailable" }, { status: 503 }); } if (url.pathname.includes("/invitations/accept")) { const adapter = await loadAdapter(); return adapter ? createDurableAuthHttpFactory(adapter, csrfSecret).acceptInvitation(request) : Response.json({ kind: "unavailable" }, { status: 503 }); } return authPost(request); } };
}
...
export function createConfiguredAuthControlPlane(environment: Record<string, string | undefined>, dependencies: { readonly sql?: AuthSql } = {}): ServerAuthControlPlane | undefined {
  ...
  return {
    admit: async (input) => {
      const riskKeyDigests = buildAuthRiskKeyDigests(hmacKey, input.risk);
