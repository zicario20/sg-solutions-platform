...
export function buildAuthRiskKeyDigests(secret: string, action: string, dimensions: Readonly<Partial<Record<"ip" | "account" | "email" | "phone" | "device" | "flow", string | undefined>>>): readonly string[] {
  if (!/^[a-z_]{3,64}$/u.test(action)) throw new Error("AUTH_RISK_ACTION_DENIED");
  const keys = (["flow", "ip", "account", "device", "email", "phone"] as const).flatMap((dimension) => dimensions[dimension]?.trim() ? [hmacIdentifier(secret, `${action}:${dimension}`, dimensions[dimension]!)] : []).slice(0, 5);
