import { type AuthLocale, authCopy, resolveAuthLocale, resolveAuthPageLocale } from "@atlas/i18n";
import { cookies } from "next/headers";
import { createElement, type ReactNode } from "react";

export type AuthPageOutcome =
  | "accepted"
  | "authenticated"
  | "denied"
  | "manual_review"
...
export async function readAuthPageContext(routeLocale?: string | null, outcome?: string | null) {
  ...
  return {
    ...
    csrf: store.get("__Host-atlas_csrf")?.value ?? "",
    sessionHandle: store.get("__Host-atlas_auth")?.value ?? "",
    outcome: resolveAuthOutcome(outcome),
  };
}
...
export function createAuthLocaleHandler() {
  return async (request: Request): Promise<Response> => {
    ...
    headers.append(
      "set-cookie",
      `${AUTH_LOCALE_COOKIE}=${locale}; Path=/; Secure; HttpOnly; SameSite=Lax; Max-Age=31536000`,
    );
