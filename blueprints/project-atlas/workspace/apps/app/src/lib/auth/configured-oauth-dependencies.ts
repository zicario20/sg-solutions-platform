...
type Provider = { authorizationUrl(input: { state: string; nonce: string; codeChallenge: string; redirectUri: string }): string; exchangeAndVerify(input: { code: string; pkceVerifier: string; expectedNonce: string; redirectUri: string }): Promise<OfficialSupabaseIdentity | undefined> };
