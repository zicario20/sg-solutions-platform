...
} from "../../lib/dashboard/ssr-admission.ts";
...
export default async function ClientPage({
  ...
  const locale = resolveDashboardLocale(
    cookieStore.get("atlas_locale")?.value,
    process.env.ATLAS_DEFAULT_LOCALE,
  );
  if ((await searchParams).availability === "unavailable")
    return <DashboardErrorView locale={locale} />;
  ...
  const result = await loadAdmittedClientDashboard(
    { sessionHandle, ...(requestedContext ? { requestedContext } : {}), locale },
    runtime,
  ...
  return (
    <>
      <DashboardView dto={result.dto} csrfToken={csrfToken} />
      {csrfToken ? (
        <DashboardAnalytics
          csrfToken={csrfToken}
          events={createDashboardAnalyticsConfig(result.dto)}
        />
      ) : null}
    </>
  );
