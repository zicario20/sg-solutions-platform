...
import { toPublicKnowledge } from "../../lib/help-content";
...
*[
  ...
  seoTitle,
  seoDescription,
  sources[]{title, authority, url, retrievedAt, effectiveDate},
  jurisdiction,
  readingMinutes,
  publishedAt
}
...
export function mapSanityPublicContent(
  ...
  const record: KnowledgeRecord = {
    ...
    category: readString(document.category, "category") as HelpCategoryId,
    slug: readString(document.slug, "slug"),
...
function readString(value: unknown, field: string): string {
  if (typeof value !== "string" || !value.trim()) throw new Error(`Invalid public content ${field}`);
  return value;
}
...
function readEnum<T extends string>(value: unknown, values: Set<T>, field: string): T {
  if (typeof value !== "string" || !values.has(value as T)) throw new Error(`Invalid public content ${field}`);
  return value as T;
}

function readPositiveInteger(value: unknown, field: string): number {
  if (!Number.isInteger(value) || (value as number) < 1) throw new Error(`Invalid public content ${field}`);
...
function readBlocks(value: unknown): KnowledgeBlock[] {
  if (!Array.isArray(value) || value.length === 0) throw new Error("Invalid public content blocks");
  return value.map((block) => {
    if (!block || typeof block !== "object") throw new Error("Invalid public content block");
    const input = block as Record<string, unknown>;
    if (input.type === "paragraph") return { type: "paragraph", text: readSafeText(input.text, "block text") };
    if (input.type === "list") {
      return { type: "list", items: readStringArray(input.items, "block items").map((item) => readSafeText(item, "block item")) };
    }
    if (input.type === "steps" && Array.isArray(input.items)) {
      return {
        ...
        items: input.items.map((item) => {
          ...
          return { title: readSafeText(step.title, "step title"), text: readSafeText(step.text, "step text") };
        }),
      };
    }
...
function readSources(value: unknown): KnowledgeSourceReference[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value)) throw new Error("Invalid public content sources");
  return value.map((source) => {
    ...
    if (!url.startsWith("https://")) throw new Error("Public source URL must use HTTPS");
    return {
